...
**Integer overflow** never wraps silently and never crashes. The bare `+`, `-`, `*`, and `**` **saturate**: a result past the signed 64-bit range pins to the nearest bound (`i64` min or max) instead of wrapping, so an accidental blow-up degrades gracefully rather than turning a large positive into a negative (the "nuclear Gandhi" class of bug). Two explicit, integer-only families opt into other behavior. The **wrapping** ops `+%`, `-%`, `*%`, `**%` do modular arithmetic on purpose (hashing, RNG, ring buffers); `**%` is exponentiation modulo `2^64`, not arbitrary `a^b mod m`. The **checked** ops `+?`, `-?`, `*?`, `**?` yield `$none` on overflow instead of a value, so the script decides: `or (+? :a :b) :cap` clamps to a fallback, `or (+? :a :b) (panic "overflow")` fails loudly, `given sum (+? :a :b) (use :sum) (recover)` branches. Everyday values never reach the 64-bit boundary, so all three behave identically there; overflow only bites in accumulation, multiplication chains, or hashing. The same saturation covers the boundary edges (`abs` of `i64` min; `i64` min divided or modded by `-1`). Float arithmetic follows IEEE 754 (overflow to `inf`), and the wrapping/checked variants are integer-only.

**The two meanings of `?`.** Position disambiguates. A *trailing* `?` marks a checked op that yields `$none` instead of failing (`+?`, `-?`, `*?`, `**?`). A *leading or standalone* `?` is randomness, from the optional `random` fragment (loaded by the CLI): `?` (random number in an inclusive range), `?<` (upper-exclusive), `??` (pick one argument at random, evaluating only the chosen one). So `*?` is "multiply, checked", never "random multiply".
